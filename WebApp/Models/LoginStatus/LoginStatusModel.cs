...
namespace Vfl.Renderer.Models.LoginStatus
{
    ...
    public class LoginStatusModel : ILoginStatusModel
    {
        ...
        public async Task<LoginStatusViewModel> GetViewModels(LoginStatusEntity entity)
        {
            ...
            if (entity.Support.HasSelectedItems())
            {
                viewModel.Support = await this.restService.GetItem<PageNodeDto>(entity.Support);
            }
            if (entity.UserDashboard.HasSelectedItems())
            {
                viewModel.UserDashboard = await this.restService.GetItem<PageNodeDto>(entity.UserDashboard);
            }
