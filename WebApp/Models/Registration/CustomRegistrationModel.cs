...
{
    public class CustomRegistrationModel : IRegistrationModel
    {
        /// <summary>
        ...
        public CustomRegistrationModel(
            ...
            IStringLocalizer<CustomRegistrationModel> localizer)
        {
            this.config = config;
        ...
        public virtual async Task<RegistrationViewModel> InitializeViewModel(RegistrationEntity entity)
        {
            ...
            {
                RegistrationHandlerPath = "api/RegistrationWidget/Register",//$"/{this.config.WebServicePath}/Registration?sf_culture={currentCulture.IetfLanguageTag}",
            ...
            if (entity.VerificationPage != null)
            {
                var item = entity.VerificationPage.ItemIdsOrdered.FirstOrDefault();
                var pageNode = await this.restService.GetItem<PageNodeDto>(item);

                string relativeUrl = uriBuilder.Uri + pageNode.ViewUrl;


                viewModel.VerificationPageUrl = relativeUrl;
            }
            if(entity.TermsConditionsPage.ItemIdsOrdered != null)
            {
                var item = entity.TermsConditionsPage.ItemIdsOrdered.FirstOrDefault();
                var pageNode = await this.restService.GetItem<PageNodeDto>(item);
                string relativeUrl = uriBuilder.Uri + pageNode.ViewUrl;
                viewModel.TermsConditionsPageUrl = relativeUrl;
            }
        ...
        private string GetPageNodeUrl(Progress.Sitefinity.Renderer.Entities.Content.MixedContentContext context)
        {
        ...
        private readonly IStringLocalizer<CustomRegistrationModel> localizer;        
    }
}
