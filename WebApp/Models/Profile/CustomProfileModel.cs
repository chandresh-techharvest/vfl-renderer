...
using VFL.Renderer.Models.Profile.Dto;
...
using VFL.Renderer.ViewModels.ProfileSettings;
using System.Linq;
...
{
    public class CustomProfileModel : ICustomProfileModel
    {
        IProfileService _profileService;
        const string endpoint = "api/MyVodafoneDashboard";        
        /// <summary>
        ...
        public CustomProfileModel(ISitefinityConfig config, IODataRestClient restService, IStyleClassesProvider styles, IHttpContextAccessor httpContextAccessor, IStringLocalizer<CustomProfileModel> localizer, IProfileService ProfileService)
        {
            this.config = config;
        ...
        /// <inheritdoc/>
        public virtual async Task<CustomProfileViewModel> InitializeViewModel(CustomProfileEntity entity)
        {
            if (entity == null)
            ...
            {
                UpdateProfileHandlerPath = $"/api/ProfileSettings", //$"/{this.config.WebServicePath}/users/updateProfile",
            ...
            var registrationSettings = await this.restService.ExecuteUnboundAction<CustomRegistrationSettingsDto>(registrationSettingsRequestArgs);
            ...
            if (registrationSettings.ActivationMethod == "AfterConfirmation" && this.httpContextAccessor.HttpContext.Request.Query.TryGetValue(EncryptedParam, out var encryptedParam))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        var emailChange = JsonConvert.DeserializeObject<CustomEmailChangeDto>(errorCodeException.Message);
            ...
            //}
            //string authCookieValue = this.httpContextAccessor.HttpContext.User.Claims.FirstOrDefault(x => x.Type == "access_token")?.Value;

            try
            {
                var profileSettingsResponse = await _profileService.GetProfileInformationAsync<ProfileSettingsResponse>();
                ...
                if (profileSettingsResponse.data != null && !string.IsNullOrEmpty(profileSettingsResponse.data.email))
                {
                    viewModel.Email = profileSettingsResponse.data.email;
                }

                viewModel.FirstName = profileSettingsResponse.data.firstName;
                viewModel.LastName = profileSettingsResponse.data.lastName;
                viewModel.MiddleName = profileSettingsResponse.data.middleName;
                //viewModel.About = user.About;
                viewModel.AvatarUrl = profileSettingsResponse.data.profileImageData;
            ...
            {
                // This will catch the exception properly
                //Console.WriteLine($"Request failed: {ex.Message}");
                throw new ApplicationException("Error calling external API", ex);
            }
            catch (Exception ex)
            {
                throw new ApplicationException("Error calling external API", ex);
        ...
        private async Task<string> GetPageNodeUrl(Progress.Sitefinity.Renderer.Entities.Content.MixedContentContext context)
        {
        ...
        private IStringLocalizer<CustomProfileModel> localizer;
    }
}
