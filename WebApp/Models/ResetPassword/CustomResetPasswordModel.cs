...
using VFL.Renderer.Models.ResetPassword.Dto;

namespace VFL.Renderer.Models.ResetPassword
{
    public class CustomResetPasswordModel : ICustomResetPasswordModel
    {
        ...
        private readonly IStringLocalizer<CustomResetPasswordModel> localizer;
        ...
        public virtual async Task<CustomResetPasswordViewModel> InitializeViewModel(CustomResetPasswordEntity entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var viewModel = new CustomResetPasswordViewModel();
            viewModel.MembershipProviderName = entity.MembershipProviderName;
            viewModel.ResetUserPasswordHandlerPath = "/api/ResetPasswordWidget/ResetUserPassword";//$"/{this.config.WebServicePath}/ResetUserPassword?sf_culture={CultureInfo.CurrentCulture.IetfLanguageTag}";
            ...
            viewModel.SendResetPasswordEmailHandlerPath = "/api/ResetPasswordWidget/SendResetPasswordEmail";//$"/{this.config.WebServicePath}/SendResetPasswordEmail?sf_culture={CultureInfo.CurrentCulture.IetfLanguageTag}";
            ...
            if (entity.VerificationPage?.Content?[0]?.Variations?.Length != 0)
            {
                var pageNodes = await this.restService.GetItems<PageNodeDto>(entity.VerificationPage, new GetAllArgs() { Fields = new[] { nameof(PageNodeDto.ViewUrl) } });
                var items = pageNodes.Items;
                if (items.Count == 1)
                    viewModel.VerificationPageUrl = items[0].ViewUrl;
            }
            ...
            if (this.IsResetPasswordRequest())
            {
                ...
                try
                {
                    var resetPasswordModel = await this.restService.ExecuteUnboundAction<CustomResetPasswordModelDto>(new BoundActionArgs()
                    {
